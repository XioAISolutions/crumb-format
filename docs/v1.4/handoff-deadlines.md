...
2. `validators/validate.js` — mirror. Same form-first dispatch: split on `T`.
   ...
   - **Datetime:** regex-test that the string ends in `Z` or matches `±HH:MM` (since `Date.parse()` is too loose under ECMAScript and accepts tz-less local time silently). After the regex check passes, parse with `Date.parse()`. **Then round-trip the calendar components** — Date.parse normalizes `2026-02-30T00:00:00Z` to `2026-03-02` without raising; pull `year/month/day/hour/min/sec` back off the Date and string-match against the input groups, treating any mismatch as malformed.
