...
1. `validators/validate.py` — new `_validate_handoff_deadlines()` walking `[handoff]` lines. For each `deadline=`:
   ...
   - **Datetime branch (contains `T`):** Python 3.10's `datetime.fromisoformat` rejects the `Z` suffix (3.11+ accepts it), and `requires-python = ">=3.10"` is in scope, so first normalize a trailing `Z` to `+00:00`: `if value.endswith("Z"): value = value[:-1] + "+00:00"`. Then parse with `datetime.fromisoformat(value)` and require `parsed.tzinfo is not None`. `fromisoformat` accepts tz-less datetimes; the grammar above rejects them, so we add the explicit check ourselves.
...
2. `validators/validate.js` — mirror. Same form-first dispatch: split on `T`.
   - **Date-only:** regex-test `^\d{4}-\d{2}-\d{2}$`, then split on `-` and construct `new Date(year, month - 1, day)`. This produces a Date at local midnight — matching the spec's "receiver-local" semantics for date-only deadlines. **Do not** prepend or append `Z`, and **do not** call `Date.parse(value)` directly — both interpret a bare `YYYY-MM-DD` as UTC midnight per the ECMAScript "date-only form" rule, which would shift the effective instant by the local offset and fire overdue checks a day early in negative-offset zones. **Add a round-trip check:** `new Date(2026, 12, 1)` does not throw — it silently rolls over to January 2027. After construction, verify `result.getFullYear() === year && result.getMonth() === month - 1 && result.getDate() === day`; if not, the date was out-of-range (e.g. month 13, Feb 30) and must be treated as malformed.
   - **Datetime:** regex-match the full form, capturing year/month/day/hour/minute/second and the offset suffix (`Z` or `±HH:MM`) into named groups. The regex itself rejects tz-less inputs (`Date.parse()` is too loose under ECMAScript and accepts those silently). **For calendar round-trip, do NOT use `Date.getHours()` / `getMonth()` / etc. on a parsed instant** — `Date` stores an absolute moment, so component getters return UTC-or-local-converted wall time, not the original offset's wall time. A valid `2026-04-30T15:00:00+02:00` would round-trip to `13:00:00` and falsely flag as malformed. Instead: pretend the captured Y/M/D/h/m/s came from UTC, build with `Date.UTC(Y, M-1, D, h, m, s)`, then compare `getUTCFullYear/getUTCMonth/getUTCDate/getUTCHours/getUTCMinutes/getUTCSeconds` of `new Date(thatTimestamp)` back against the captured groups. Offset correctness is enforced by the regex; calendar correctness by the UTC round-trip; they're independent concerns.
   - Same warn-not-reject behavior as the Python branch.
