...
API hardening release. CRUMB 0.7.0 tightens the public surface with a structured error taxonomy, a formal JSON Schema for the parsed-document shape, an auto-generated CLI reference with drift detection, and a widened CI matrix (Python 3.10 → 3.13). No file-format changes — every `v=1.1` document that worked under 0.6 still works, and every caller that caught `ValueError` from `parse_crumb()` still works unchanged.

### New Features

**Structured error taxonomy (`cli.errors`)**
- New `CrumbError` base class (subclass of `ValueError` for backwards compatibility) with a stable machine-readable `code` attribute
- Ten named subclasses, one per parse failure site: `MissingMarkerError`, `MissingEndMarkerError`, `MissingSeparatorError`, `InvalidHeaderLineError`, `MissingHeaderError`, `BadVersionError`, `UnknownKindError`, `OrphanBodyError`, `MissingSectionError`, `EmptySectionError`
- Ten stable error-code constants (`E_MISSING_MARKER`, `E_BAD_VERSION`, etc.) exported as strings — third-party tooling may compare against them
- `parse_crumb()` now raises the specific subclass at each failure site; string messages are preserved exactly so fixture `.expected.txt` files, CLI output, and log entries are unchanged

**JSON Schema for parsed shape (`schemas/crumb.schema.json`)**
- Formal Draft 2020-12 schema describing the dict returned by `parse_crumb(text)` — complements the text-format ABNF in SPEC.md §2.1
- Required headers (`v`, `kind`, `source`) pinned; `kind` enum locked to the six frozen kinds; `v` enum pinned to `1.1`
- Namespaced extension headers (`ext.<ns>.<name>.v<N>`, `x-<name>`) admitted via pattern properties
- Stable contract for the v=1.x line — new optional headers and sections may be added in minor releases

**Auto-generated CLI reference (`docs/CLI.md`)**
- 674-line reference covering all 41 top-level subcommands and every nested subcommand (palace, passport, policy, audit, bridge, webhook, template)
- Generated by `tools/generate_cli_reference.py` from the live argparse tree — single source of truth, no hand-maintained duplication
- `tools/generate_cli_reference.py --check` mode for CI drift detection; `test_cli_reference.py` fails the build if the CLI surface changes without a regen

**Python 3.13 support + CI drift gate**
- `.github/workflows/tests.yml` matrix widened from 3.10/3.11/3.12 to 3.10/3.11/3.12/3.13 with `fail-fast: false`
- CI now installs the `[test]` extra (pinned pytest + hypothesis ranges from 0.5.0), not bare pytest
- New CI step verifies `docs/CLI.md` has no drift vs. the argparse tree

### Improvements

- 436 tests passing (up from 391 in v0.6.0): +24 error-taxonomy tests, +18 JSON-schema conformance tests, +3 CLI-reference drift tests
- `docs/STABILITY.md` lists `cli.errors` and the error codes as stable 0.7.0 public API; `schemas/crumb.schema.json` noted as the parsed-shape contract
- `tests/test_roundtrip_and_fuzz.py` hypothesis strategy hardened: post-filters blank-after-strip values so the generator never emits effectively-empty sections
- `crumb-format` runtime package still has **zero required dependencies** — JSON Schema validation in tests uses a small hand-rolled structural checker

### No breaking changes

Every CLI command, MCP tool, REST endpoint, Python API, and `.crumb` file accepted by 0.6 is still accepted by 0.7. Callers that catch `ValueError` from `parse_crumb()` continue to work — the new `CrumbError` classes subclass `ValueError` and the error message strings are preserved byte-for-byte.
