...
- **`crumb hello`** — 30-second walkthrough. Prints a working sample
  task crumb, validates it, copies it to the clipboard if a clipboard
  tool is available, then lists three concrete next steps. Lands a
  brand-new install at "I have a working crumb in my clipboard"
  without any flags. `--no-clipboard` skips the copy.
