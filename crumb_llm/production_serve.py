...
def serve_production(
    ...
    ProductionHandler.model = model
    ProductionHandler.tokenizer = tok
    ProductionHandler.queue = RequestQueue(max_concurrent)
    ProductionHandler.model_info = {
        "arch": "crumb_llm",
        "version": "0.2.0",
        "params": n_params,
    ...
    if index_dir:
        ...
        ProductionHandler.index = idx
        print(f"[prod] indexed {len(idx.sections)} sections from {index_dir}")
...
def main(argv: list[str] | None = None) -> None:
    ap = argparse.ArgumentParser(description="Crumb LLM Production Server")
    ...
    serve_production(args.ckpt, args.port, args.host, args.index, args.max_concurrent)
